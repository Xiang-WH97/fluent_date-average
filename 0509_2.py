...
class GeometryAnalyzer:
    ...
    def _fit_plane_and_project(self):
        x = self.df[self.coord_cols[0]].values
        y = self.df[self.coord_cols[1]].values
        z = self.df[self.coord_cols[2]].values
        ...
        axes_map = {'X': np.array([1, 0, 0]), 'Y': np.array([0, 1, 0]), 'Z': np.array([0, 0, 1])}
        len_vec = axes_map[self.length_axis]
        grav_vec = axes_map[self.gravity_axis]

        def project_to_plane(vec):
            return vec - np.dot(vec, normal) * normal

        u_base = project_to_plane(len_vec)
        u_norm = np.linalg.norm(u_base)
        if u_norm < 1e-6:
            raise ValueError(f"管长方向 {self.length_axis} 与平面法向量垂直，无法投影")
        u_base = u_base / u_norm

        v_base = project_to_plane(grav_vec)
        v_base = v_base - np.dot(v_base, u_base) * u_base
        v_norm = np.linalg.norm(v_base)
        if v_norm < 1e-6:
            raise ValueError(f"重力方向 {self.gravity_axis} 与管长方向在平面内共线")
        v_base = v_base / v_norm

        self.plane_centroid = centroid
        self.u_base = u_base
        self.v_base = v_base

        vec = points - centroid
        dist = np.dot(vec, normal)[:, np.newaxis]
        proj_points = points - dist * normal

        u_coords = np.dot(proj_points - centroid, u_base)
        v_coords = np.dot(proj_points - centroid, v_base)

        self.coord1_vals = u_coords
        self.coord2_vals = v_coords

        self.df_plane = self.df.copy()
        self.plane_x_label = f"{self.length_axis} (投影)"
        self.plane_y_label = f"{self.gravity_axis} (投影)"
        self.plane_label = f"拟合平面 (法向量: {normal[0]:.3f}, {normal[1]:.3f}, {normal[2]:.3f})"
        self.plane_axes = [self.length_axis + '_proj', self.gravity_axis + '_proj']
...
class DivisionVisualizationWindow(tk.Toplevel):
    def __init__(self, parent, processor, region_info):
    ...
    def create_visualization(self):
        ...
        if hasattr(self.processor, 'invert_u') and self.processor.invert_u:
            ax.invert_xaxis()
        if hasattr(self.processor, 'invert_v') and self.processor.invert_v:
            ax.invert_yaxis()
...
class Application(tk.Tk):
    ...
    def setup_divide_page(self, parent):
        ...
        self.swap_btn.grid(row=2, column=1, pady=5)

        tk.Button(parent, text="预览划分线", command=self.preview_division, bg="#e0e0e0").grid(row=3, column=1, pady=5)

        self.division_viz_frame = tk.Frame(parent, height=300, width=650)
        self.division_viz_frame.grid(row=4, column=0, columnspan=4, padx=5, pady=10)
        ...
        self.compute_btn.grid(row=5, column=1, pady=20)

        self.divide_status = tk.Label(parent, text="请先在页面1中识别四边形", fg="blue")
        self.divide_status.grid(row=6, column=0, columnspan=4, pady=10)
    ...
    def identify_geometry(self):
        ...
        if quad_type == 'plane':
            ...
            try:
                ...
                self.geo_info_text.delete(1.0, tk.END)
                self.geo_info_text.insert(tk.END, info)
                self.divide_status.config(text=f"当前四边形周长：{params.get('fit_perimeter',0):.4f}")
                self.visualize_shape()
                self.preview_division()
